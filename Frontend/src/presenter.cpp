...
class EntityFactory {
 public:
  static Message createMessage(long long      chat_id,
                               long long      sender_id,
                               const QString& text,
...
void Presenter::initialConnections() {
  ...
  connect(
      message_delegate_.get(), &MessageDelegate::unreadMessage, this, &Presenter::onUnreadMessage);
}
...
MessageDelegate* Presenter::getMessageDelegate() {
  if (!message_delegate_)
    message_delegate_ =
        std::make_unique<MessageDelegate>(manager_->dataManager(), manager_->tokenManager());
...
void Presenter::onScroll(
    int value) {  // todo: multithreaded event changed current_opened_chat_id_ (?)
...
void Presenter::newMessage(Message& msg) {
  ...
  if (current_opened_chat_id_.has_value() && current_opened_chat_id_ == msg.chat_id &&
      max == value) {
...
void Presenter::sendButtonClicked(const QString& text_to_send) {
  ...
  auto message_to_send = EntityFactory::createMessage(
      *current_opened_chat_id_, current_user_->id, trimmed_text, QUuid::createUuid().toString());
...
std::vector<Message> Presenter::getListOfMessagesBySearch(const QString& prefix) {
  ...
  auto list_of_messages_of_chat =
      manager_->dataManager()->getMessageModel(*current_opened_chat_id_)->messages();
