...
QNetworkReply *NetworkAccessManager::put(const QNetworkRequest &req,
                                         const QByteArray &byte_array) {
  return net_manager_.put(req, byte_array);
}

QNetworkReply *NetworkAccessManager::del(const QNetworkRequest &req) {
  return net_manager_.deleteResource(req);
}
