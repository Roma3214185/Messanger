...
Model::Model(const QUrl &url, INetworkAccessManager *netManager, ICache *cash,
             ISocket *socket, DataManager *data_manager)
    : cache_(cash), chat_model_(std::make_unique<ChatModel>()),
      user_model_(std::make_unique<UserModel>()), data_manager_(data_manager),
      token_manager_(std::make_unique<TokenManager>()),
      socket_use_case_(std::make_unique<SocketUseCase>(
          std::make_unique<SocketManager>(socket, url))),
      chat_use_case_(std::make_unique<ChatUseCase>(
          std::make_unique<ChatManager>(netManager, url), data_manager_,
          chat_model_.get(), token_manager_.get())),
      user_use_case_(std::make_unique<UserUseCase>(
          data_manager_, std::make_unique<UserManager>(netManager, url),
          token_manager_.get())),
      message_use_case_(std::make_unique<MessageUseCase>(
          data_manager_, std::make_unique<MessageManager>(netManager, url),
          token_manager_.get())),
      session_use_case_(std::make_unique<SessionUseCase>(
          std::make_unique<SessionManager>(netManager, url))) {
  LOG_INFO("[Model::Model] Initialized Model with URL: '{}'",
           url.toString().toStdString());
}

void Model::setupConnections() {
  connect(data_manager_, &DataManager::chatAdded, this,
          [this](const ChatPtr &added_chat) {
  ...
  connect(data_manager_, &DataManager::messageAdded, this,
          [this](const Message &message) {
            user_use_case_->getUserAsync(message.sender_id);
            auto last_message =
                getMessageModel(message.chat_id)->getLastMessage();
  ...
  connect(data_manager_, &DataManager::messageDeleted, this,
          [this](const Message &deleted_message) {
