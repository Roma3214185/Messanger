...
//     j["read"].at("read_by_me").get_to(m.read.read_by_me); if
//     (j["read"].contains("count")) j["read"].at("count").get_to(m.read.count);
//   }

//   if(j.contains("reactions")) {
//     if (j["reactions"].contains("counts"))
//     j["reactions"].at("counts").get_to(m.reactions.counts); if
//     (j.contains("reactions") &&
