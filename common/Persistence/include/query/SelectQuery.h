...
private:
  void saveEntityInCache(const T &entity,
                         std::chrono::hours ttl = std::chrono::hours(24)) const;
  int getEntityId(
      const T &entity) const; // todo: make concept requires there is field id
  [[nodiscard]] QString buildQuery() const override;
  auto getGenerations() const;
  [[nodiscard]] std::optional<std::vector<T>>
  tryLoadFromCache(const std::string &key) const;
