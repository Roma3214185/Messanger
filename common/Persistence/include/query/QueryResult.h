...
template <EntityJson T> struct SelectResult {
  std::vector<T> result;
  std::string error;
};

template <EntityJson T> struct DeleteResult {
  bool success{false};
  int affected_rows{0};
  std::string error;
};
...
template <typename V, typename Variant> struct is_in_variant;

template <typename V, typename... Ts>
struct is_in_variant<V, std::variant<Ts...>>
    : std::disjunction<std::is_same<V, Ts>...> {};
