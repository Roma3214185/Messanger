...
struct ChatMember final {
  ...
  ChatMember(long long chat_id, long long user_id,
             long long added_at = utils::time::getCurrentTime(), std::string status = "test_status")
      : chat_id(chat_id), user_id(user_id),
      status(std::move(status)), added_at(added_at)
  {
...
template <> struct adl_serializer<ChatMember> {
  static void to_json(nlohmann::json &json_chat_member,
                      const ChatMember &chat_member) {
    json_chat_member =
        nlohmann::json{{ChatMemberTable::ChatId, chat_member.chat_id},
  ...
  static void from_json(const nlohmann::json &json_chat_member,
