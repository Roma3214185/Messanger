...
template <> struct adl_serializer<Reaction> {
  static void to_json(nlohmann::json &json_message,
                      const Reaction &user_reaction) {
  ...
  static void from_json(const nlohmann::json &json_message,
...
template <> struct adl_serializer<UserMessage> {
  ...
  static void from_json(const nlohmann::json &j, UserMessage &m) {
    ...
    if (j.contains("read")) {
      if (j["read"].contains("read_by_me"))
        j["read"].at("read_by_me").get_to(m.read.read_by_me);
