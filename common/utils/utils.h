...
public:
  ...
  constexpr details::value_or_reference_return_t<T> get() const noexcept(
      noexcept(details::value_or_reference_return_t<T>(std::declval<T &>()))) {
    return ptr_;
  }
...
constexpr auto
operator<(const not_null<T> &lhs,
          const not_null<U> &rhs) noexcept(noexcept(std::less<>{}(lhs.get(),
                                                                  rhs.get())))
...
template <class T, class U>
constexpr auto
operator<=(const not_null<T> &lhs, const not_null<U> &rhs) noexcept(
