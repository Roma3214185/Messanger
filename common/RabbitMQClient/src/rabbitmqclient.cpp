...
RabbitMQClient::RabbitMQClient(const RabbitMQConfig &rabit_mq_config,
                               IThreadPool *pool)
...
void RabbitMQClient::declareExchange(const std::string &exchange,
  ...
  try {
    auto channel = AmqpClient::Channel::Create(
        rabit_mq_config_.host, rabit_mq_config_.port, rabit_mq_config_.user,
...
void RabbitMQClient::publish(const PublishRequest &publish_request) {
  LOG_INFO("Publish: {} | {}", publish_request.exchange,
           publish_request.routing_key);
  declareExchange(publish_request.exchange, publish_request.exchange_type,
                  false);
  try {
    auto channel = AmqpClient::Channel::Create(
        rabit_mq_config_.host, rabit_mq_config_.port, rabit_mq_config_.user,
    ...
    LOG_INFO(
        "[rabbit] Try to publish message '{}' to exchange '{}' with key '{}'",
        publish_request.message, publish_request.exchange,
        publish_request.exchange_type);
    channel->BasicPublish(publish_request.exchange, publish_request.routing_key,
                          msg);
...
void RabbitMQClient::subscribe(const SubscribeRequest &subscribe_request,
  ...
  LOG_INFO("Subscrive: {} | {}", subscribe_request.exchange,
           subscribe_request.routing_key);
  running_ = true;

  auto consumer_thread = std::thread([=, this]() {
    try {
      declareExchange(subscribe_request.exchange,
                      subscribe_request.exchange_type, false);
      auto channel = AmqpClient::Channel::Create(
          rabit_mq_config_.host, rabit_mq_config_.port, rabit_mq_config_.user,
          rabit_mq_config_.password);
      // channel->DeclareExchange(subscribe_request.exchange,
      // subscribe_request.exchangeType, true, false, false);
      channel->DeclareQueue(subscribe_request.queue, false, false, false,
                            false);
      channel->BindQueue(subscribe_request.queue, subscribe_request.exchange,
                         subscribe_request.routing_key);

      LOG_INFO("[rabbit] Queue '{}' bound to exchange '{}' with key '{}'",
               subscribe_request.queue, subscribe_request.exchange,
               subscribe_request.routing_key);

      const std::string consumer_tag = channel->BasicConsume(
          subscribe_request.queue, "", false, false, false);
      LOG_INFO("[rabbit] Subscribed to '{}':'{}' in queue '{}'",
               subscribe_request.exchange, subscribe_request.routing_key,
               subscribe_request.queue);

      while (running_) {
        AmqpClient::Envelope::ptr_t envelope;
        if (!channel->BasicConsumeMessage(consumer_tag, envelope, 200))
          continue;
