...
JWTVerifier::JWTVerifier(const std::string &public_key_path,
                         const std::string &issuer)
    : public_key_(
          readFile(public_key_path)), // todo: can be excaption in readFile
      verifier_(
          jwt::verify()
...
JWTVerifier::verifyTokenAndGetUserId(const std::string &token) {
  try {
    auto decoded = jwt::decode(token);
    verifier_.verify(decoded);

    long long user_id =
        std::stoll(decoded.get_payload_claim("sub").as_string()); // todo: try
                                                                  // catch
    return user_id;
