...
GatewayServer::GatewayServer(GatewayApp&      app,
    ...
    : app_(app), provider_(provider), cache_(cache), proxy_(client), pool_(pool), queue_(queue) {}
...
void GatewayServer::registerRoute(const std::string& base_path, int port) {
  ...
  app_.route_dynamic(base_path).methods(
      "GET"_method, crow::HTTPMethod::DELETE, crow::HTTPMethod::PUT)(
...
void GatewayServer::handlePostRequest(
  ...
  RequestDTO request_info = getRequestInfo(req, path);
  cache_->set(
      "request:" + request_info.request_id, "{ \"status\": \"queued\" }", std::chrono::seconds(30));
...
void GatewayServer::subscribeOnNewRequest() {
  ...
  queue_->subscribe(
      subscribe_request, [this](const std::string& event, const std::string& payload) {
        ...
        auto result               = proxy_.forward(request_info, port);
        LOG_INFO(
            "Finished result in queue_->subscribe, request_info->request_id = {}, status_code = "
            "{}, body = {}",
        ...
        cache_->set("request_id:" + request_info.request_id,
                    ...
                    std::chrono::seconds(30));
        cache_->set(
            "request_body:" + request_info.request_id,
...
void GatewayServer::registerWebSocketRoutes() {
  std::string backend_url =
      fmt::format("ws://127.0.0.1:{}/ws", provider_->ports().notificationService);
  ...
  CROW_WEBSOCKET_ROUTE(app_, "/ws")
      ...
      .onmessage(
          [ws_bridge, this](crow::websocket::connection& client, const std::string& data, bool) {
            ws_bridge->onClientMessage(client, data);
            // metrics_->newMessage(client.get_remote_ip());
          })
      .onclose([ws_bridge, this](
                   crow::websocket::connection& client, const std::string& reason, uint16_t code) {
