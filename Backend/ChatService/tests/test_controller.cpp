...
TEST_CASE("Test createPrivateChat") {
  ...
  SECTION("Chat not created expected serverError status code and problem issue") {
    ...
    REQUIRE(body == utils::details::formError("Failed to create chat"));
  }
...
TEST_CASE("Test getAllChatMembers") {
  ...
  SECTION("DB returns empty list expected serverError") {
    ...
    REQUIRE(code == Config::StatusCodes::serverError);
    REQUIRE(body == utils::details::formError("Error in db.getMembersOfChat"));
  }
...
TEST_CASE("Test ChatController::GetChat") {
  TestController::TestFixture fix;
  std::string chat_id = "12";

  SECTION("Invalid token expected info about it") {
    fix.mock_autoritized->need_fail = true;
    auto [code, body] = fix.controller.getChat(fix.req, chat_id);
    CHECK(code == Config::StatusCodes::unauthorized);
    CHECK(body == utils::details::formError(Config::IssueMessages::invalidToken));
  }
  ...
  auto now = utils::time::getCurrentTime();
  Chat private_chat;
  private_chat.id = std::stoi(chat_id);
  ...
  SECTION("Not found other user for private chat") {
    auto [code, body] = fix.controller.getChat(fix.req, chat_id);
    REQUIRE(code == Config::StatusCodes::notFound);
    REQUIRE(body == utils::details::formError("Other user not found for this chat"));
  }
...
TEST_CASE("Test ChatController::GetAllChats") {
  ...
  SECTION("Invalid token → userError and invalidToken message") {
    ...
    auto [code, body] = fix.controller.getAllChats(fix.req);

    REQUIRE(code == Config::StatusCodes::userError);
    REQUIRE(body == utils::details::formError(Config::IssueMessages::invalidToken));
  }
  ...
  private_chat.id = 1;
  private_chat.is_group = false;
  private_chat.created_at = utils::time::getCurrentTime();
