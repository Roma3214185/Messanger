...
void NotificationManager::handleMessage(const std::string& body) {
    auto res = nlohmann::json::parse(body);
    if (res["event"] == "saved") {
        if (res["saved"] == "User") onUserSaved();
        else if (res["saved"] == "Message") {
            Message newMsg;
            from_json(res, newMsg);
            onMessageSaved(newMsg);
        }
        else if (res["saved"] == "MessageStatus") onMessageStatusSaved();
        else LOG_ERROR("Unknown type of saved entity");
    }
}
