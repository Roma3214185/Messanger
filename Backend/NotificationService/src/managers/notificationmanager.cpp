...
void NotificationManager::subscribeMessageSaved() {
  ...
  mq_client_->subscribe(
      request, [this, rout_config](const std::string& event, const std::string& payload) {
        LOG_INFO("Subscribe save message received message to save: event {} and payload {}",
                 event,
                 payload);
...
void NotificationManager::subscribeMessageDeleted() {
  ...
  mq_client_->subscribe(
      request, [this, rout_config](const std::string& event, const std::string& payload) {
        LOG_INFO("Subscribe on deleted message received message to delete: event {} and payload {}",
                 event,
                 payload);
...
void NotificationManager::subscribeMessageStatusSaved() {
  ...
  mq_client_->subscribe(
      request, [this, rout_config](const std::string& event, const std::string& payload) {
        LOG_INFO(
            "Subscribe on message status saved received: event {} and payload {}", event, payload);
...
void NotificationManager::handleOnMessageDeleted(const std::string& payload) {
  ...
  LOG_INFO(
      "For chat id '{}' finded '{}' members", deleteted_message.chat_id, members_of_chat.size());
...
bool NotificationManager::notifyMember(long long         user_id,
...
void NotificationManager::saveDeliveryStatus(const Message& msg,
  ...
  LOG_INFO(
      "saveDeliveryStatus message {} for receiver id {}", nlohmann::json(msg).dump(), receiver_id);
