...
[[nodiscard]] Response sendResponse(int code, const std::string &text,
                                    bool is_error) {
  ...
  LOG_INFO("Return responce {} and body {}, is_error {}", code, text,
...
std::optional<std::string> fetchTag(const RequestDTO &req) {
  ...
  return it != req.url_params.end() ? std::make_optional(it->second)
                                    : std::nullopt;
}
...
AuthController::AuthController(IAuthManager *manager,
                               IAutoritizer *authoritizer,
                               IGenerator *generator, IConfigProvider *provider)
    : manager_(manager), authoritizer_(authoritizer), generator_(generator),
      provider_(provider) {} // todo(roma) make itokengenrator (ISRP)

Response AuthController::loginUser(const RequestDTO &req) {
  auto body = crow::json::load(req.body);
  if (!body || !body.count("email") ||
      !body.count(
          "password")) { // todo: make function bool contains(json, field)
    return sendResponse(provider_->statusCodes().badRequest, "Invalid Json",
                        true);
  }
  ...
  return sendResponse(provider_->statusCodes().success,
                      userToJson(*logged_user, token).dump(), false);
}
...
Response AuthController::handleMe(const RequestDTO &req) {
  ...
  if (!user) {
    ...
    return sendResponse(provider_->statusCodes().notFound,
                        provider_->issueMessages().userNotFound, true);
  }

  return sendResponse(provider_->statusCodes().success,
                      userToJson(*user, req.token).dump(), false);
}
...
Response AuthController::findByTag(const RequestDTO &req) {
  ...
  return sendResponse(provider_->statusCodes().success, json_users.dump(),
...
Response AuthController::findById(const RequestDTO & /*req*/,
  ...
  if (!found_user) {
    LOG_ERROR("User with id {} not found", *user_id);
    return sendResponse(provider_->statusCodes().notFound, "User not found",
                        true);
  }

  auto user_json = userToJson(*found_user);
  return sendResponse(provider_->statusCodes().success,
                      user_json["user"].dump(), false);
}

AuthController::OptionalId
AuthController::verifyToken(const std::string &token) {
...
Response AuthController::registerUser(const RequestDTO &req) {
  ...
  if (!body || !body.count("email") || !body.count("password") ||
      !body.count("name") || !body.count("tag")) {
  ...
  LOG_INFO(
      "Registe user (email: {}) | (password : {}) | (name : {}) | (tag: {})",
      register_request.email, register_request.password, register_request.name,
  ...
  if (!register_user) {
    ...
    return sendResponse(provider_->statusCodes().userError,
                        "User already exist", true);
  }
  LOG_INFO("User registered successfully, id is {}", register_user->id);
  std::string token = generator_->generateToken(register_user->id);
  LOG_INFO("User user {}, id {} token is {}", register_user->username,
           register_user->id, token);
  return sendResponse(provider_->statusCodes().success,
                      userToJson(*register_user, token).dump(), false);
}
