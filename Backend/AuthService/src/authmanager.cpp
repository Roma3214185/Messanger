...
OptionalUser AuthManager::findUserByEmail(const std::string &email) {
  ...
  auto query =
      QueryFactory::createSelect<User>(rep_.getExecutor(), rep_.getCache());  // TODO: remove executor and cache;
...
OptionalUser AuthManager::registerUser(const RegisterRequest &req) {
  ...
  if (user_with_same_email) {
    LOG_WARN(
        "There is user with email {} already, his email {} name is {} and "
        "tag is {}",
        req.email, user_with_same_email->email, user_with_same_email->username, user_with_same_email->tag);
    return std::nullopt;
  }
  ...
  if (users_with_same_tag) {
    LOG_WARN(
        "There is user with tag {} already, his email {} name is {} and "
        "tag is {}",
        req.tag, users_with_same_tag->email, users_with_same_tag->username, users_with_same_tag->tag);
    return std::nullopt;
  }

  LOG_INFO("User can be saved");

  User user_to_save(generator_->generateId(), req.name, req.email, req.tag);
