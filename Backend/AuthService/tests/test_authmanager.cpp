...
struct TestAuthManagerFixture {
  ...
  TestAuthManagerFixture() : rep(db, &executor, cache, &pool), manager(rep, &generator) {
    user.id       = user_id;
    user.email    = email;
    user.tag      = tag;
...
TEST_CASE("Test AuthManager::register") {
  ...
  RegisterRequest        reg_req{
             .email = fix.email, .password = fix.password, .name = fix.username, .tag = fix.tag};
  fix.manager.mock_user_by_email = std::nullopt;
  ...
  //   std::string expected_sql = "INSERT OR REPLACE INTO credentials (user_id, hash_password)
  //   VALUES (?, ?)"; auto res = fix.manager.registerUser(reg_req);
