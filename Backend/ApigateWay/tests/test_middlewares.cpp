...
TEST_CASE("Test Authmiddleware") {
  ...
  auto doCallAfter = [&]() {
    fix.auth_middleware.after_handle(fix.req, fix.res, fix.auth_ctx, fix.dummy_parent_ctx);
  };
...
TEST_CASE("Test RateLimitMiddleware") {
  ...
  SECTION("Section rate_limit not allowed expected return ratelimitexceed code and message") {
    doCallBefore();
