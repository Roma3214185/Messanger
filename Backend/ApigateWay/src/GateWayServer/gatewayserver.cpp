...
GatewayServer::GatewayServer(const int& port)
    : port_(port),
      ...
      notificationProxy_(
          getenv_or("PAYMENT_SERVICE_URL", "http://localhost:8080")) {
  registerRoutes();
}
...
void GatewayServer::registerMessagesRoutes() {
  ...
  ([this](const crow::request& req, crow::response& res, string path) {
    ...
    auto result =
        messageProxy_.forward(req, downstream_path, getMethod(req.method));
    res.code = result.first;
    res.write(result.second);
    res.end();
  });
...
void GatewayServer::registerNotificationRoutes() {
  ...
  ([this](const crow::request& req, crow::response& res, string path) {
    ...
    auto result =
        notificationProxy_.forward(req, downstream_path, getMethod(req.method));
    res.code = result.first;
    res.write(result.second);
    res.end();
  });
