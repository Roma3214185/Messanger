...
void RabbitMQClient::subscribe(const std::string& queue,
                               const std::function<void(const std::string&)>& callback)
{
    channel_->DeclareQueue(queue, false, true, false, false);
    std::string consumerTag = channel_->BasicConsume(queue, "", true, false, false);
    running_ = true;

    consumerThread_ = std::thread(&RabbitMQClient::consumerLoop, this, consumerTag, callback);
}

void RabbitMQClient::consumerLoop(const std::string& consumerTag,
                                  std::function<void(const std::string&)> callback)
{
    while (running_) {
        AmqpClient::Envelope::ptr_t envelope = channel_->BasicConsumeMessage(consumerTag);
        if (envelope) {
